...
export default function Card({ className = "", children, ...rest }: CardProps) {
  return (
      <section
          {...rest}
          className={`rounded-xl border border-ink-200/70 bg-white [box-shadow:var(--shadow-e1)] transition-shadow duration-200 hover:[box-shadow:var(--shadow-e2)] ${className}`}
      >
        {children}
      </section>
  );
